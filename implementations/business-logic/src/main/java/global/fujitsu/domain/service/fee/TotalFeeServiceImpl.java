...
public class TotalFeeServiceImpl implements TotalFeeService {
    ...
    public TotalFeeResponse getBaseFee(TotalFeeRequest request) {
        ...
        return toTotalFeeResponse(
            airTemperatureFeeService.getBaseFee(new GetAirTemperatureFeeRequest(vehicleTypeId, measurement.airTemperature())),
            weatherPhenomenonFeeService.getBaseFee(new GetWeatherPhenomenonFeeRequest(vehicleTypeId, measurement.weatherPhenomenon())),
            regionalBasedFeeService.getBaseFee(new GetRegionalBasedFeeRequest(vehicleTypeId, measurement.regionId())),
            windSpeedFeeService.getBaseFee(new GetWindSpeedFeeRequest(vehicleTypeId, measurement.windSpeed()))
        );
    }
